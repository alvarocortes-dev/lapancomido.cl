...
must_haves:
  truths:
    ...
    - "El catálogo público sigue funcionando exactamente igual"
  artifacts:
...
<critical_constraint>
⚠️ NO MODIFICAR ENDPOINTS QUE USA EL FRONTEND ACTUAL

Los siguientes endpoints están EN USO por el catálogo público. NO tocar:
- /products (GET - listado y detalle)
- /categories (GET - listado)
- /store (GET - configuración tienda)
- /contact (POST - guardar leads)
- /product-images (GET - imágenes)

Solo eliminar rutas/controladores de auth, orders, address, favorites, gateways, location, user.
</critical_constraint>
...
<task type="auto">
  ...
  <verify>
    ...
    # Verificar que la base de datos está accesible y sincronizada
    cd packages/database && npx prisma db pull --print 2>&1 | head -10 && echo "OK: database accessible"
    ```
  </verify>
  <done>
