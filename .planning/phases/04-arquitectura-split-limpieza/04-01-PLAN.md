...
must_haves:
  truths:
    ...
    - "La UI del usuario final NO cambia (HomePage, CatalogPage, QuotationModal intactos)"
  artifacts:
...
<critical_constraint>
⚠️ NO MODIFICAR LA UI ACTUAL DEL USUARIO FINAL

Los siguientes componentes están PULIDOS y FUNCIONANDO. NO tocar:
- HomePage (slider, grid, elígenos)
- CatalogPage (productos, categorías, paginación)
- ProductPage (detalle de producto)
- Header, Footer, MainLayout
- QuotationModal, SelectionBar
- ContactPage

Este plan SOLO elimina archivos legacy NO USADOS. Si un archivo está siendo importado
por cualquier componente activo, NO eliminarlo.
</critical_constraint>
...
<task type="auto">
  ...
  <files>
    ...
    apps/web/src/components/RegisterForm.jsx
    apps/web/src/components/ProductForm.jsx
    apps/web/src/components/Marques.jsx
    apps/web/src/layouts/MainLayout.jsx (editar, no eliminar)
  </files>
  <action>
    ...
    rm apps/web/src/components/Marques.jsx
    ```
    ...
    - Marques.jsx → Banner promocional removido (solo retorna null)
    
    **IMPORTANTE:** Editar `apps/web/src/layouts/MainLayout.jsx` para quitar el import y uso de Marques:
    - Eliminar línea: `import { Marques } from "../components/Marques";`
    - Eliminar línea: `<Marques />`
  </action>
  <verify>
    ```bash
...
- UI del usuario final IDÉNTICA (HomePage, CatalogPage, QuotationModal sin cambios)
</success_criteria>
