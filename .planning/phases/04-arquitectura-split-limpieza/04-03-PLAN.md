...
must_haves:
  truths:
    - "lapancomido.cl sirve el sitio público"
  ...
  artifacts:
    - path: "apps/web/vercel.json"
      provides: "Configuración Vercel para web público"
    - path: "apps/admin/vercel.json"
      provides: "Configuración Vercel para admin"
    - path: "apps/api/vercel.json"
      provides: "Configuración Vercel para API serverless"
...
<objective>
Configurar subdominios separados en Vercel para web público, admin y API, con CORS correctamente configurado.

Purpose: Arquitectura split que permite deploys independientes y aislamiento de seguridad entre sitio público y admin.
Output: Tres proyectos Vercel desplegados en subdominios, CORS funcionando.
</objective>

<critical_constraint>
⚠️ NO MODIFICAR LA UI DEL SITIO PÚBLICO

El sitio público (lapancomido.cl) debe verse EXACTAMENTE igual después del deploy:
- HomePage, CatalogPage, ProductPage sin cambios visuales
- QuotationModal, SelectionBar funcionando igual
- Solo se agregan configuraciones de infraestructura (vercel.json, CORS)

El shell de admin es un placeholder nuevo, no afecta al sitio público.
</critical_constraint>
...
<task type="auto">
  ...
  <action>
    Editar `apps/api/src/server.js` para configurar CORS dinámico que permita requests de los subdominios.
    
    1. Buscar la configuración actual de CORS (probablemente `app.use(cors())` o similar).
    
    2. Reemplazar con configuración específica para subdominios:
    
    ```javascript
    const cors = require('cors');
    
    // Orígenes permitidos según ambiente
    ...
    IMPORTANTE: Asegurarse de que esta configuración esté ANTES de las rutas.
  </action>
  <verify>
    ```bash
  ...
  <done>
    CORS configurado para permitir lapancomido.cl, admin.lapancomido.cl en producción y localhost en desarrollo.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    1. Crear/actualizar `apps/web/vercel.json` para el sitio público:
    ...
    4. Eliminar el vercel.json antiguo de la raíz si existe (la arquitectura ahora es split):
    ```bash
  ...
  <done>
    3 archivos vercel.json creados, uno para cada app con configuración apropiada.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    1. Mejorar `apps/admin/index.html` con título correcto:
    ...
    Buscar cómo se configura la URL base de la API. Si está hardcodeada, cambiar a:
    ```javascript
  ...
  <verify>
    ...
    grep "Próximamente" apps/admin/src/main.jsx && echo "OK: admin placeholder"
    npm run --prefix apps/admin build && echo "OK: admin builds"
    ```
  </verify>
  <done>
    - Admin shell con branding correcto
    - Configuración de env vars para API URL
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
    Configuración completa de subdominios:
  ...
  <how-to-verify>
    **Verificación local:**
    ...
    3. Verificar que el catálogo funciona en http://localhost:3001
    
    **Deploy a Vercel (requiere configuración manual):**
    
    El usuario debe:
    1. Crear proyecto Vercel para `apps/api` → Asignar dominio `api.lapancomido.cl`
    2. Crear proyecto Vercel para `apps/admin` → Asignar dominio `admin.lapancomido.cl`
    3. Actualizar proyecto existente de `apps/web` → Verificar dominio `lapancomido.cl`
    ...
    **Verificar post-deploy:**
    1. https://lapancomido.cl carga el catálogo
    2. https://admin.lapancomido.cl muestra placeholder "Próximamente disponible"
    3. https://api.lapancomido.cl/api/test responde JSON
    4. El catálogo puede cargar productos (CORS funciona)
  </how-to-verify>
  <resume-signal>
    Escribir "verified" cuando los 3 subdominios estén funcionando, o describir cualquier problema encontrado.
  </resume-signal>
</task>
...
5. CORS en server.js permite subdominios de producción
6. Admin shell se renderiza con placeholder correcto
</verification>

<success_criteria>
- Los 3 subdominios responden correctamente:
  - lapancomido.cl → Catálogo público
  - admin.lapancomido.cl → Placeholder admin
  - api.lapancomido.cl → API endpoints
